...
def main():
    ...
    if args.command == "add":
        ...
        target_index = 0
        target_id = args.id
        found = False # whether the id of the specified task was found
        ...
        for index, task in enumerate(task_list):
            if task["id"] == target_id:
                ...
                break
        if not found:
            raise Exception("No task with specified ID found")
        
        task_list.pop(target_index)
        json_manipulation.save_tasks(task_list, filename = "tasks.json") # update task file
